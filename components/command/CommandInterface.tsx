...
-5          3.69899      host node2
 ...
 5    hdd   1.23300          osd.5   up   1.00000  1.00000`,
            'success',
         );
...
• Cluster performance: Optimal`,
            'success',
         );
      } else if (lowerCommand.includes('predict')) {
         addCommand(
            command,
            `🔮 ML Prediction Engine activated...
...
POOLS:
  ...
  cephfs_data             3   64  384 GiB   98.4k  1.1 TiB   9.84    3.6 TiB`,
            'success',
         );
      } else {
         addCommand(
...
Type 'help' for available commands.`,
   ...
   return (
      <div className="bg-black/40 backdrop-blur-sm border border-green-500/30 rounded-xl p-4 font-mono text-green-400">
         ...
         <div ref={terminalRef} className="h-64 overflow-y-auto mb-4 p-2 bg-black/20 rounded border scrollbar-thin scrollbar-thumb-green-500/30">
            <AnimatePresence>
               {commands.map(cmd => (
                  <motion.div key={cmd.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-4">
                     ...
                     <pre
                        className={`text-sm whitespace-pre-wrap pl-4 ${
                           cmd.status === 'error' ? 'text-red-400' : cmd.status === 'warning' ? 'text-yellow-400' : 'text-green-300'
                        }`}
                     >
                        {cmd.output}
                     </pre>
               ...
               {[
                  ...
                  <motion.button
                     ...
                     className={`p-4 rounded-xl border backdrop-blur-sm transition-all duration-300 ${
                        activeMode === mode.key ? 'border-cyan-400/50 bg-cyan-400/10 shadow-cyan-400/20' : 'border-white/10 bg-white/5 hover:border-white/20'
                     }`}
            ...
            >
               ...
               {activeMode === 'neural' && (
                  <div className="p-6">
                     ...
                     <div className="min-h-[600px] h-[930px] bg-black/20 rounded-lg relative overflow-hidden">
