...
import { createRef, forwardRef, memo, RefObject, useCallback, useEffect, useMemo, useRef, useState } from 'react';
import SimpleBar from 'simplebar-react';
import 'simplebar-react/dist/simplebar.min.css';
import * as THREE from 'three';
import { Canvas, useFrame, useThree } from '@react-three/fiber';
import { Box, Environment, OrbitControls, Plane, Stars, Text as Text3D, Torus } from '@react-three/drei';
...
import BangingGlassBall from '@/components/models/BangingGlassBall';
...
>(({ poolData, position = [0, 0, 0], textures, onPoolClick, selectedPoolIdRef, animationRefs }, ref) => {
...
// R3F JSX PGNode Component - Optimized with memo
const PGNode = memo(
   forwardRef<
      THREE.Group,
      {
         pgData: any;
         position?: [number, number, number];
         onClick?: (pgData: any) => void;
         selectedPGIdRef?: RefObject<string | null>;
      }
   >(({ pgData, position = [0, 0, 0], onClick, selectedPGIdRef }, ref) => {
      const groupRef = useRef<THREE.Group>(null);
      ...
      // HUD colors
      const hudColor = pgData.state === 'active+clean' ? '#00d2ff' : pgData.state === 'degraded' ? '#ffaa00' : '#ff3333';

      // Holographic HUD Ring shader
      const hudRingShader = useMemo(() => {
         return {
            uniforms: {
               time: { value: 0 },
      ...
      // Energy Field shader (healthy PGs only)
      const energyFieldShader = useMemo(() => {
         if (pgData.state !== 'active+clean') return null;
      ...
      // Warning ring shader (unhealthy PGs only)
      const warningRingShader = useMemo(() => {
         if (pgData.state === 'active+clean') return null;

         return {
            uniforms: {
               time: { value: 0 },
               color: { value: new THREE.Color(pgData.state === 'degraded' ? '#ffaa00' : '#ff3333') },
      ...
      useFrame(({ clock }) => {
         ...
         // Update HUD ring shader time
         if (groupRef.current) {
            const hudRing = groupRef.current.getObjectByName('hud-ring');
            if (hudRing && (hudRing as THREE.Mesh).material) {
               const material = (hudRing as THREE.Mesh).material as THREE.ShaderMaterial;
               if (material.uniforms) material.uniforms.time.value = time;
            }

            // Update HUD panels shader time
            const hudPanels = groupRef.current.getObjectsByProperty('name', 'hud-panel');
            hudPanels.forEach(panel => {
               if (panel && (panel as THREE.Mesh).material) {
                  const material = (panel as THREE.Mesh).material as THREE.ShaderMaterial;
                  if (material.uniforms) material.uniforms.time.value = time;
               }
            });

            // Update energy field shader time (healthy PGs only)
            if (pgData.state === 'active+clean') {
               const energyField = groupRef.current.getObjectByName('energy-field');
               if (energyField && (energyField as THREE.Mesh).material) {
                  const material = (energyField as THREE.Mesh).material as THREE.ShaderMaterial;
                  if (material.uniforms) material.uniforms.time.value = time;
               }
            }

            // Update warning ring shader time (unhealthy PGs only)
            if (pgData.state !== 'active+clean') {
               const warningRing = groupRef.current.getObjectByName('health-ring');
               if (warningRing && (warningRing as THREE.Mesh).material) {
                  const material = (warningRing as THREE.Mesh).material as THREE.ShaderMaterial;
                  if (material.uniforms) material.uniforms.time.value = time;
               }
            }
         }
      ...
      return (
         <group ref={groupRef} position={position} onClick={handleClick} userData={{ ...pgData, type: 'PG' }} frustumCulled={false}>
            {/* Main PG Mesh with deformed geometry */}
            <mesh geometry={deformedGeometry} castShadow receiveShadow>
               <meshPhysicalMaterial {...material} />
            </mesh>

            {/* Holographic HUD */}
            <group name="pg-holographic-hud">
               {/* Main HUD Ring */}
               <mesh name="hud-ring" position={[0, 4, 0]} rotation={[-Math.PI / 2, 0, 0]}>
                  <ringGeometry args={[3.5, 4.0, 64]} />
                  <shaderMaterial {...hudRingShader} transparent side={THREE.DoubleSide} depthWrite={false} />
               </mesh>
               ...
               {Array.from({ length: 3 }, (_, i) => {
                  ...
                  const radius = 3.5;
                  const panelPosition: [number, number, number] = [Math.cos(angle) * radius, 3.8, Math.sin(angle) * radius];

                  return (
                     <mesh key={i} name="hud-panel" position={panelPosition} lookAt={[0, 3.8, 0]}>
                        <planeGeometry args={[1.0, 0.3]} />
                        <shaderMaterial {...hudPanelShader} transparent side={THREE.DoubleSide} />
                     </mesh>
                  );
               })}
            </group>
            ...
            {/* 3D Text Label */}
            <Text3D position={[0, 4, 0]} fontSize={2} color="#ffffff" anchorX="center" anchorY="bottom" outlineWidth="8%" outlineColor="#000000">
               {pgData.id}
            </Text3D>

            {/* Energy Field (healthy PGs only) */}
            {pgData.state === 'active+clean' && energyFieldShader && (
               <mesh name="energy-field">
                  <sphereGeometry args={[3.2, 32, 32]} />
                  <shaderMaterial {...energyFieldShader} transparent blending={THREE.AdditiveBlending} side={THREE.DoubleSide} depthWrite={false} />
               </mesh>
            )}

            {/* Warning Ring (unhealthy PGs only) */}
            {pgData.state !== 'active+clean' && warningRingShader && (
               <mesh name="health-ring">
                  <ringGeometry args={[3.1, 3.5, 64, 1, 0, Math.PI * 2]} />
                  <shaderMaterial {...warningRingShader} transparent opacity={0.8} side={THREE.DoubleSide} depthWrite={true} />
               </mesh>
            )}
         </group>
      );
...
const DoughnutTable = ({ position = [0, -40, 0] }: { position?: [number, number, number] }) => {
   ...
   const thickness = 5; // Y-axis thickness
   const layers = 30; // Increased from 5 to 20 for smoother thickness
   const useLayer = false;
   if (!useLayer) {
      return (
         <group position={position} rotation={[0, 0, 0]} castShadow receiveShadow>
            ...
            <RoundMirrorTable
               ...
               segment={256}
            />
            {/*<group position={[innerRadius - 10, 12, 0]}>
               <pointLight args={['#ffffff', 20, 20, 1]} position={[0, 10, 0]} castShadow />
               <MetaPlate animate scale={7} castShadow />
            </group>*/}
            <Text3D
               scale={10}
   ...
   return (
      <group position={position} rotation={[-Math.PI / 2, 0, 0]} castShadow receiveShadow>
         {/* Create multiple layers for thickness */}
         {Array.from({ length: layers }, (_, layerIndex) => {
            const layerY = (layerIndex / (layers - 1)) * thickness - thickness / 2;
            // Inner part slightly higher - create slope effect
            const innerHeight = layerY + Math.abs(layerY) * 0.3;
            // Calculate opacity for seamless blending
            const centerLayer = Math.floor(layers / 2);
            const distanceFromCenter = Math.abs(layerIndex - centerLayer);
            const opacity = Math.max(0.4, 1.0 - (distanceFromCenter / centerLayer) * 0.6);

            return (
               <group key={`layer-${layerIndex}`}>
                  {/* Main doughnut structure */}
                  {Array.from({ length: segments }, (_, i) => {
                     const startAngle = (i / segments) * Math.PI * 2;
                     const endAngle = ((i + 1) / segments) * Math.PI * 2;

                     return (
                        <mesh key={`segment-${i}-${layerIndex}`} position={[0, 0, layerY]} frustumCulled={false}>
                           <ringGeometry args={[innerRadius, outerRadius, 32, 1, startAngle, endAngle - startAngle]} />
                           <meshPhysicalMaterial
                              ...
                              transparent
                              side={THREE.DoubleSide}
                              depthWrite={layerIndex === centerLayer}
                           />
                        </mesh>
                     );
                  })}
                  ...
                  {/*도너츠 안쪽 y축 면*/}
                  <Torus args={[innerRadius - 1.25, 0.2, 8, 64]} position={[0, 0, innerHeight + 1.5]}>
                     <meshPhysicalMaterial color={0x000000} metalness={1} roughness={0} emissive={0x171717} emissiveIntensity={0.8} side={THREE.DoubleSide} />
                  </Torus>
                  {/*도너츠 바깥쪽 y축 면*/}
                  <Torus args={[outerRadius + 1, 0.2, 8, 64]} position={[0, 0, innerHeight - 0.5]}>
                     <meshPhysicalMaterial color={0x1e293b} metalness={0.8} roughness={0.2} side={THREE.DoubleSide} />
                  </Torus>
               </group>
            );
         })}

         {/* Inner doughnut - smaller, brighter, slightly higher */}
         {Array.from({ length: layers }, (_, layerIndex) => {
            const innerDoughnutInnerRadius = 56;
            const innerDoughnutOuterRadius = 68.8;
            const layerY = (layerIndex / (layers - 1)) * thickness - thickness / 2 + 2.5; // +1.5 higher, uniform width

            // Calculate opacity for seamless blending
            const centerLayer = Math.floor(layers / 2);
            const distanceFromCenter = Math.abs(layerIndex - centerLayer);
            const opacity = Math.max(0.5, 1.0 - (distanceFromCenter / centerLayer) * 0.5);

            return (
               <group key={`inner-layer-${layerIndex}`}>
                  {Array.from({ length: segments }, (_, i) => {
                     const startAngle = (i / segments) * Math.PI * 2;
                     const endAngle = ((i + 1) / segments) * Math.PI * 2;

                     return (
                        <mesh key={`inner-segment-${i}-${layerIndex}`} position={[0, 0, layerY]} frustumCulled={false}>
                           <ringGeometry args={[innerDoughnutInnerRadius, innerDoughnutOuterRadius, 32, 1, startAngle, endAngle - startAngle]} />
                           <meshPhysicalMaterial
                              color={0x111827} // Brighter than main doughnut (0x1e293b)
                              metalness={0.9}
                              roughness={0.1}
                              emissive={0x334155}
                  ...
                  {/* Inner edge ring for inner doughnut */}
                  <Torus args={[innerDoughnutInnerRadius, 0.15, 8, 64]} position={[0, 0, layerY - 0.5]}>
                     <meshPhysicalMaterial color={0x6366f1} metalness={1} roughness={0} emissive={0x6366f1} emissiveIntensity={0.5} side={THREE.DoubleSide} />
                  </Torus>
               </group>
            );
         })}

         {/* Outer doughnut segment divider lines */}
         <group position={[0, 0, thickness / 2]}>
            {Array.from({ length: segments }, (_, i) => {
               const angle = (i / segments) * Math.PI * 2;

               // Start point (outer doughnut inner radius)
               const startX = Math.cos(angle) * innerRadius;
               const startY = Math.sin(angle) * innerRadius;

               // End point (outer doughnut outer radius)
               ...
               return (
                  <line key={`outer-divider-${i}`}>
                     ...
                     <lineBasicMaterial color={0x94a3b8} opacity={0.8} transparent linewidth={2} />
                  </line>
               );
            })}
         </group>

         {/* Cyber neon edge lines on top surface */}
         <group position={[0, 0, thickness / 2 + 0.1]}>
            ...
            {/* Outer edge neon line */}
            {/*<mesh>
               <torusGeometry args={[outerRadius, 0.2, 8, 64]} />
               <meshStandardMaterial color={0x06b6d4} emissive={0x06b6d4} emissiveIntensity={1.0} transparent opacity={0.9} />
            </mesh>*/}
         </group>
      </group>
   );
...
// Animated Bubble Component - Interactive deforming sphere with mouse tracking
const AnimatedBubble = ({
   ...
   // Create geometry with original vertices stored
   const geometry = useMemo(() => {
      const geo = new THREE.SphereGeometry(30, 32, 32); // Smaller size for doughnut center

      // Store original positions
      const positions = geo.attributes.position.array as Float32Array;
      originalVertices.current = [];

      for (let i = 0; i < positions.length; i += 3) {
         originalVertices.current.push(new THREE.Vector3(positions[i], positions[i + 1], positions[i + 2]));
      }
   ...
   useEffect(() => {
      if (animationRefs) {
         animationRefs.meshRef.current = meshRef.current;
         animationRefs.geometryRef.current = geometryRef.current;
         animationRefs.originalVertices.current = originalVertices.current;
         animationRefs.mousePos.current = mousePos.current;
         animationRefs.springScale.current = springScale.current;
         animationRefs.scale = scale;
      }
   }, [animationRefs, scale]);

   const handlePointerDown = () => {
      // Animate scale down on click with GSAP-like easing
      const animate = () => {
         let progress = 0;
         ...
         const tick = () => {
            ...
            springScale.current = 1 - eased * 0.3; // scale to 0.7
   ...
   const handlePointerUp = () => {
      // Animate scale back up with elastic effect
      const animate = () => {
         let progress = 0;
         const duration = 900; // ms
         const startTime = Date.now();
         const startScale = springScale.current;
         ...
         const tick = () => {
            ...
            // Elastic ease out
            const eased = 1 - Math.pow(2, -10 * progress) * Math.sin(((progress * 10 - 0.75) * (2 * Math.PI)) / 3);
            springScale.current = startScale + (1 - startScale) * eased;
   ...
   return (
      <mesh ref={meshRef} position={position} onPointerDown={handlePointerDown} onPointerUp={handlePointerUp} castShadow>
         <primitive object={geometry} />
         <meshStandardMaterial color="#bd4be3" emissive="#bd4be3" emissiveIntensity={0.4} roughness={0.61} metalness={0.21} transparent={false} />
      </mesh>
   );
};
...
>(({ hostData, segmentIndex, position, animationRefs, hostPlaneRefs, selectedHostIdRef }, ref) => {
   ...
   useFrame(({ clock }, delta) => {
      ...
      if (pointLightRef.current) {
         const isSelected = selectedHostIdRef?.current === hostData.name;
         pointLightRef.current.intensity = isSelected ? 10 : 0.3;
      }
   });
   ...
   return (
      <>
         ...
         >
            ...
            <pointLight ref={pointLightRef} args={['#ffffff', 10, 50, -0.3]} position={[0, 30, 0]} castShadow />
            {/* Control panel frame */}
            {/*<mesh>
...
const CrystalOSD = ({
   ...
   return (
      <group ref={groupRef} position={position} userData={{ type: 'OSD', id: osdData.id, osdData }}>
         ...
         {/* OSD ID label */}
         <Text3D position={[0, -3, 0]} fontSize={1.4} color={Colors.white} anchorX="center" anchorY="middle" outlineColor={Colors.black} outlineWidth="20%">
            OSD.{osdData.id}
         </Text3D>
      </group>
   );
...
const ClusterTopologyScene = ({
   ...
   const calculatePGPositionsAroundPool = (pool: any, pgs: any[]): any[] => {
      const poolPosition = pool.position;
      const minRadius = 20; // Minimum distance from pool
      const maxRadius = 250; // Maximum boundary constraint
      const pgCount = pgs.length;

      return pgs.map((pg, index) => {
         // Create multiple concentric circles for better distribution
         const circleIndex = Math.floor(index / 24); // 24 PGs per circle
         const positionInCircle = index % 24;

         // Expand radius for each circle
         const baseRadius = minRadius + circleIndex * 25;
         const radiusVariation = Math.random() * 15 - 7.5; // ±7.5 variation
         const distance = Math.min(baseRadius + radiusVariation, maxRadius - 10);

         // Calculate angle with some randomness
         const baseAngle = (positionInCircle / 24) * Math.PI * 2;
         const angleVariation = (Math.random() - 0.5) * 0.3; // Small angle variation
         const angle = baseAngle + angleVariation;

         let x = poolPosition.x + Math.cos(angle) * distance;
         let z = poolPosition.z + Math.sin(angle) * distance;
         let y = poolPosition.y - 20 - Math.random() * 5; // Below pool with variation
   ...
   // PG rendering will be handled directly in showPGsForPool function
   ...
   // Sync selectedPoolIdRef with selectedPoolId state for React re-renders
   useEffect(() => {
      const checkSelectedPoolChange = () => {
         if (selectedPoolIdRef.current !== selectedPoolId) {
            setSelectedPoolId(selectedPoolIdRef.current);
         }
      };

      // Check immediately and then periodically
      checkSelectedPoolChange();
      const interval = setInterval(checkSelectedPoolChange, 50);
   ...
   return (
      <>
         ...
         <directionalLight position={[100, 80, 60]} intensity={3} />
         {/*<directionalLight position={[-30, 30, -40]} intensity={1.5} castShadow shadow-mapSize={[2048, 2048]}>
            <orthographicCamera args={[-100, 100, -100, 100, 0.1, 200]} />
         </directionalLight>*/}
         <directionalLight position={[0, -50, 0]} intensity={5}>
            {/*<orthographicCamera args={[-100, 100, -100, 100, 0.1, 200]} />*/}
         </directionalLight>
         ...
         <TextSphere ref={textSphereRef} position={[0, -36, 0]} scale={4} text="OKESTRO" bgColor={Colors.pink[300]} textColor={Colors.blue[500]} />
         ...
         {/* Render PG Nodes for selected pool using JSX */}
         {selectedPoolId &&
            (() => {
               const selectedPool = mockTopologyData.pools.find(p => p.id === selectedPoolId);
               if (!selectedPool?.pgs) return null;

               // Calculate pool position for PG positioning
               const layoutManager = new AdaptiveLayoutManager();
               const allPools = mockTopologyData.pools.map((p, i) => ({ index: i }));
               const positions = layoutManager.applyLayout(allPools, 'hierarchical', { spacing: 20 });
               const poolIndex = mockTopologyData.pools.findIndex(p => p.id === selectedPoolId);
               const poolPos = positions[poolIndex].position;

               const tempPool = { position: { x: poolPos[0], y: 40, z: poolPos[2] } };
               const positionedPGs = calculatePGPositionsAroundPool(tempPool, selectedPool.pgs);

               // Limit to first 100 PGs to avoid performance issues
               const limitedPGs = positionedPGs.slice(0, 100);

               return limitedPGs.map((pgData: any) => (
                  <PGNode
                     key={pgData.id}
                     pgData={pgData}
                     position={pgData.position || [0, 0, 0]}
                     onClick={() => {
                        if (selectedPGIdRef.current === pgData.id) {
                           selectedPGIdRef.current = null;
                        } else {
                           selectedPGIdRef.current = pgData.id;
                           highlightNode(pgData);
                        }
                     }}
...
export default function ClusterTopologyViewFail2() {
   ...
   // PG related refs (removed unused refs)
   ...
   const pulseNode = (node: THREE.Group | THREE.Object3D) => {
      ...
      switch (searchTypeRef.current) {
         ...
         case 'osd':
            osdNodesRef.current.forEach(node => {
               ...
               const osdIdStr = String(osdId).toLowerCase();
               const osdFullName = `osd.${osdIdStr}`;

               // Check if query matches OSD ID (supports partial matching)
               if (osdIdStr.includes(query) || osdFullName.includes(query) || query.includes(osdIdStr)) {
                  pulseNode(node);
                  matchCount++;
               }
            });
   ...
   // PG 노드 생성 (Deprecated - Replaced by R3F JSX PGNode component)
   // This function is kept temporarily for reference and will be removed
   ...
   // OSD들을 Host 위에 Grid Layout으로 배치
   /*const positionOSDsOnHost = (host: THREE.Group, osds: any[]): THREE.Group[] => {
      const osdNodes: THREE.Group[] = [];
      const hostPos = host.position;
      const gridSize = Math.ceil(Math.sqrt(osds.length));
      const spacing = 10;
      ...
      const textureLoader = new THREE.TextureLoader();
      textureLoader.load('/3d/textures/wood/brick_diffuse.jpg', function (map) {
      ...
      textureLoader.load('/3d/textures/wood/brick_bump.jpg', function (map) {
         map.wrapS = THREE.RepeatWrapping;
         map.wrapT = THREE.RepeatWrapping;
         map.anisotropy = 4;
         map.repeat.set(0.1, 0.1);
      ...
      textureLoader.load('/3d/textures/wood/brick_roughness.jpg', function (map) {
         map.wrapS = THREE.RepeatWrapping;
         map.wrapT = THREE.RepeatWrapping;
         map.anisotropy = 4;
         map.repeat.set(0.1, 0.1);
      ...
      osds.forEach((osdData, index) => {
         const row = Math.floor(index / gridSize);
         const col = index % gridSize;

         const x = hostPos.x + (col - (gridSize - 1) / 2) * spacing;
         const z = hostPos.z + (row - (gridSize - 1) / 2) * spacing;
         const y = -37.5; // On top of doughnut surface

         const osdNode = createOSDNode(osdData, material);
         osdNode.position.set(x, y, z);
         osdNodes.push(osdNode);
         sceneRef.current!.add(osdNode);
      });
            ...
            if (osdNode) {
               ...
               gsap.to(osdNode.position, {
                  ...
                  onComplete: () => {
                     ...
                     setTimeout(() => {
                        const particles = createTrafficParticles(pgPosition, osdPosition, 0x00d2ff);
   ...
   const showPGsForPool = (poolId: number, poolRefs?: RefObject<any>[]): void => {
      ...
      console.log(`Pool ${poolId} selected for PG rendering`);

      // Create connection lines between pool and OSDs
      createPoolOSDConnections(poolId, poolRefs);
      // Create connection lines between pool and PGs
      createPoolPGConnections(poolId);
   };
   ...
   const calculatePGPositionsAroundPool = (pool: any, pgs: any[]): any[] => {
      ...
      return pgs.map((pg, index) => {
         ...
         let x = poolPosition.x + Math.cos(angle) * distance;
         let z = poolPosition.z + Math.sin(angle) * distance;
         let y = poolPosition.y - 10 - Math.random() * 5; // Below pool with variation
   ...
   // PG nodes will be rendered using JSX in ClusterTopologyScene

   // Create connection lines between pool and related OSDs
   const createPoolOSDConnections = (poolId: number, poolRefs?: RefObject<any>[]): void => {
      const selectedPool = mockTopologyData.pools.find(p => p.id === poolId);
      if (!selectedPool) return;

      // Get all OSDs used by this pool's PGs
      const usedOSDs = new Set<number>();
      selectedPool.pgs?.forEach(pg => {
         pg.acting.forEach((osdId: number) => usedOSDs.add(osdId));
      });
      ...
      const positions = layoutManager.applyLayout(allPools, 'hierarchical', { spacing: 20 });
      const poolIndex = mockTopologyData.pools.findIndex(p => p.id === poolId);
      const basePos = positions[poolIndex].position;
      const poolPos = [basePos[0], 40, basePos[2]]; // Pool Y position is 40

      // Create connections to OSDs
      Array.from(usedOSDs).forEach(osdId => {
         const osd = mockTopologyData.osds.find(o => o.id === osdId);
         if (!osd) return;

         const host = mockTopologyData.hosts.find(h => h.id === osd.host);
         if (!host) return;

         // Calculate actual OSD position based on host layout
         const hostIndex = mockTopologyData.hosts.findIndex(h => h.id === host.id);
         const hostOsds = mockTopologyData.osds.filter(o => o.host === host.id);
         const osdIndexInHost = hostOsds.findIndex(o => o.id === osdId);

         // Use same layout as SpaceshipHost component
         const segmentCount = mockTopologyData.hosts.length;
         const segmentAngle = (2 * Math.PI) / segmentCount;
         const radius = 150;
         const angle = hostIndex * segmentAngle;

         // Host position
         const hostX = Math.cos(angle) * radius;
         const hostZ = Math.sin(angle) * radius;
         const hostY = -30;

         // OSD position relative to host (grid formation)
         const cols = Math.ceil(Math.sqrt(hostOsds.length));
         const spacing = 6;
         const row = Math.floor(osdIndexInHost / cols);
         const col = osdIndexInHost % cols;

         const osdX = hostX + (col - (cols - 1) / 2) * spacing;
         const osdZ = hostZ + (row - (cols - 1) / 2) * spacing;
         const osdY = hostY - 5;

         console.log(
            `Creating connection from pool ${poolId} to OSD ${osdId} at host ${host.id}: pool(${poolPos[0]},${poolPos[1]},${poolPos[2]}) -> osd(${osdX},${osdY},${osdZ})`,
         );

         // Create connection line
         const line = createAnimatedFlowConnection(new THREE.Vector3(poolPos[0], poolPos[1], poolPos[2]), new THREE.Vector3(osdX, osdY, osdZ), 0x00ff88, 2);
   ...
   // Create connection lines between pool and related PGs
   const createPoolPGConnections = (poolId: number): void => {
      const selectedPool = mockTopologyData.pools.find(p => p.id === poolId);
      if (!selectedPool?.pgs) return;
      ...
      // Calculate PG positions (same logic as JSX rendering)
      const tempPool = { position: { x: poolPos[0], y: 40, z: poolPos[2] } };
      const positionedPGs = calculatePGPositionsAroundPool(tempPool, selectedPool.pgs);

      // Limit to first 100 PGs (same as JSX rendering)
      const limitedPGs = positionedPGs.slice(0, 100);

      // Create connections to PGs
      limitedPGs.forEach(pgData => {
         const pgPos = pgData.position;

         console.log(
            `Creating connection from pool ${poolId} to PG ${pgData.id}: pool(${poolPos[0]},${poolPos[1]},${poolPos[2]}) -> pg(${pgPos[0]},${pgPos[1]},${pgPos[2]})`,
         );

         // Create connection line from pool to PG
         const line = createAnimatedFlowConnection(
            new THREE.Vector3(poolPos[0], poolPos[1], poolPos[2]),
            new THREE.Vector3(pgPos[0], pgPos[1], pgPos[2]),
            0x00aaff, // Blue color for pool-PG connections
            1.5,
         );
   ...
   // generatePGsForPool function removed - using direct pool.pgs access
         ...
         if (node.userData.type === 'OSD') {
            ...
            if (mesh && mesh.material) {
               ...
               if (animData && (animData as any).isAnimating) {
                  ...
                  // Floating animation for non-selected OSDs only
   ...
   return (
      <>
         ...
         <div className={`topology-container bg-black`}>
            ...
            <button onClick={toggleSearchPanel} className="search-expand-btn top-[96px]" style={{ transform: 'translateX(0px)', opacity: 1 }}>
               <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <circle cx="6" cy="6" r="4" stroke="currentColor" strokeWidth="1.5" />
                  <path d="m10 10 2 2" stroke="currentColor" strokeWidth="1.5" />
               </svg>
