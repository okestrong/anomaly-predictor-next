...
export const ThroughputChart: React.FC<ThroughputChartProps> = ({ timeRange = '1h', autoRefresh = true, className }) => {
   const throughput = useDashboardStore(useShallow(state => state.throughput));
   ...
   const loadData = async (): Promise<ECOption> => {
      ...
      const timeLabels = read.map((d, index) => {
         if (index % Math.ceil(read.length / 5) === 0) {
            return new Date(d.timestamp).toLocaleTimeString('ko-KR', {
               hour12: false,
               hour: '2-digit',
               minute: '2-digit',
            });
   ...
   const renderFooter = () => {
      const throughputData = throughput;
      const data = {
         read: throughputData?.readThroughput || [],
         write: throughputData?.writeThroughput || [],
      };
