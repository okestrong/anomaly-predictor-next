...
import { Card, Button, FuturisticSpinner } from '@/components/common';
...
export default function CapacityStatus() {
   ...
   // Extract capacity data from dashboard store
   const capacity = dashboardData?.capacity;
   const totalCapacity = formatBytes(capacity?.totalBytes || 0);
   const usedCapacity = formatBytes(capacity?.usedBytes || 0);
   const availableCapacity = formatBytes(capacity?.availableBytes || 0);
   const usagePercentage = capacity?.usagePercent || 0;
   const dailyGrowth = formatBytes(capacity?.dailyGrowthBytes || 0);
   const timeToFull = capacity?.timeToFullDays ? `${capacity.timeToFullDays} days` : 'N/A';

   // Pool colors for visualization
   const poolColors = ['#00d2ff', '#ff0080', '#00ff41', '#8b5cf6', '#fbbf24', '#f97316', '#06b6d4', '#84cc16'];

   const pools: Pool[] =
      capacity?.pools?.map((pool, index) => ({
         name: pool.name,
         used: formatBytes(pool.usedBytes),
         percentage: pool.usagePercent,
         type: pool.type === 'replicated' ? 'Replicated' : 'Erasure',
         color: poolColors[index % poolColors.length],
      })) || [];

   const trendData = capacity?.usageTrend || [0];
      ...
      <Card
         ...
         header={
            <div className="flex items-center justify-between">
               ...
               <Button size="xs" variant="cyber" onClick={refreshCapacity} disabled={isLoading}>
      ...
      >
         {isLoading ? (
            <div className="flex items-center justify-center h-full">
               <FuturisticSpinner size="lg" label="Loading storage data..." />
            </div>
         ) : (
            <>
               ...
               <div className="text-center mb-6 relative z-10 pt-4">
                  <div className="relative inline-block">
                     ...
                     <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="text-2xl font-bold text-white">{usagePercentage.toFixed(1)}%</span>
               ...
               <div className="space-y-3 mb-6 relative z-10 px-4">
                  ...
                  {pools.map(pool => (
                     <div key={pool.name} className="space-y-2">
                        ...
                        <div className="flex items-center justify-between text-xs text-secondary-400">
                           <span>{pool.percentage.toFixed(2)}% used</span>
                           <span>{pool.type}</span>
                        </div>
                     </div>
                  ))}
               ...
               <div className="space-y-3 relative z-10 px-4">
                     ...
                     {trendData.length > 0 ? (
                        trendData.map((value, index) => (
                           <div
                              ...
                              style={{
                                 height: `${value}%`,
                                 width: `calc(100% / ${trendData.length})`,
                        ...
                        <div className="flex items-center justify-center w-full h-12 text-xs text-secondary-400">
                  ...
                  {trendData.length > 0 && (
                     <div className="flex justify-between text-xs text-secondary-400">
                        <span>{trendData.length} days ago</span>
                        <span>Today</span>
                     </div>
                  )}
               </div>
