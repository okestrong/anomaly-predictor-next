   ...
   return (
      <group ref={rackRef}>
         ...
         <mesh castShadow receiveShadow position={[-w / 2, 0, 0]}>
            ...
            {/*<meshStandardMaterial color={0x0b1220} metalness={0.9} roughness={0.25} />*/}
            <meshStandardMaterial map={tex?.rackSide} color={0xffffff} metalness={0.6} roughness={0.5} />
         </mesh>
         {/* 우측면 */}
         <mesh castShadow receiveShadow position={[w / 2, 0, 0]}>
            <boxGeometry args={[frameThick, h, d]} />
         ...
         {/* 옆면 텍스처 - castShadow 추가 */}
         {/*{tex?.rackSide ? (
            <>
               <mesh castShadow receiveShadow position={[-w / 2 - 0.01, 0, 0]} rotation={[0, Math.PI / 2, 0]}>
                  <planeGeometry args={[d, h]} />
                  <meshStandardMaterial map={tex.rackSide} metalness={0.6} roughness={0.5} side={THREE.DoubleSide} />
               </mesh>
               <mesh castShadow receiveShadow position={[w / 2 + 0.01, 0, 0]} rotation={[0, -Math.PI / 2, 0]}>
                  <planeGeometry args={[d, h]} />
                  <meshStandardMaterial map={tex.rackSide} metalness={0.6} roughness={0.5} side={THREE.DoubleSide} />
            ...
            <>
                텍스처가 없을 때 기본 색상으로 표시
               <mesh castShadow receiveShadow position={[-w / 2 - 0.01, 0, 0]} rotation={[0, Math.PI / 2, 0]}>
                  <planeGeometry args={[d, h]} />
                  <meshStandardMaterial color={0x1f2937} metalness={0.6} roughness={0.5} side={THREE.DoubleSide} />
               </mesh>
               <mesh castShadow receiveShadow position={[w / 2 + 0.01, 0, 0]} rotation={[0, -Math.PI / 2, 0]}>
                  <planeGeometry args={[d, h]} />
                  <meshStandardMaterial color={0x1f2937} metalness={0.6} roughness={0.5} side={THREE.DoubleSide} />
...
const Table = ({
   ...
   useEffect(() => {
      (async () => {
         ...
         texturesRef.current.rackSide = await loadTexture('/3d/textures/rack/rack-side.jpg');
